...
class WaxController
{
     ...
     * @param string $host
     */
    public function redirect_to($options, $protocol = false, $status = 302, $host = null)
    {
        if (!$host) {
        ...
        switch (true) {
            case is_array($options):
                $url = $protocol . $host . UrlHelper::url_for($options);
    ...
    public function render_view()
    {
        if ($this->use_plugin) {
            WaxLog::log("info",
                "[DEPRECATION] use_plugin in controllers is deprecated, use the add_plugin method intead.");
            $this->add_plugin($this->use_plugin);
        }
        if ($this->shared_plugin) {
            WaxLog::log("info",
                "[DEPRECATION] shared_plugin in controllers is deprecated, use the add_plugin method intead.");
    ...
    public function render_layout()
    {
        ...
        foreach ((array)Autoloader::view_paths("plugin") as $path) {
            $layout->add_path($path . "layouts/" . $this->use_layout);
        }
        ob_end_clean();
    ...
    {
        if (is_readable($path . "." . $format)) {
            return true;
        }
        if ($plugin) {
            $file_path = PLUGIN_DIR . $path . "." . $format;
        } else {
