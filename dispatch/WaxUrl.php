...
class WaxUrl
{
   ...
   **/
    ...
    static public $defaults = ["controller" => "page", "action" => "index"];
     ...
     * @return void
     */
    static public function map($pattern, $outcome = [], $conditions = [])
    {
        array_unshift(self::$mappings, [$pattern, $outcome, $conditions]);
    }
    ...
    **/
    static public function compile()
    {
        ...
        if (!isset(self::$params["format"]) && preg_match("/(.*)\.(.*)/", self::$uri, $matches)) {
        ...
        foreach (self::$mappings as &$map) {
            ...
            if (preg_match("#$map[0]#", self::$uri, $matches)) {
                if (isset($map[1]["controller"])) {
                    self::$params["controller"] = $map[1]["controller"];
                }
    ...
    static public function build($options = [])
    {
        ...
        if (isset($options["controller"])) {
            ...
            $url[] = rtrim(self::$params["controller"], "/");
        }
        if (isset($options["action"])) {
            $url[] = $options["action"];
        } elseif (self::$params["action"]) {
            $url[] = self::$params["action"];
        }
        if (isset($options["id"])) {
            $url[] = $options["id"];
        } elseif (self::$params["id"]) {
            $url[] = self::$params["id"];
        }
    ...
    static public function build_url($options = [])
    {
    ...
    {
        if (!$input && preg_match("/(.*)\.(.*)/", self::$uri, $matches) && $matches[2]) {
            $input = $matches[1];
        } // drop out format to route multilevel controllers with format correctly
        if (!$input) {
            $route = explode("/", self::$params["route"]);
        } else {
            $route = explode("/", $input);
        }
        $controller = false;
        while (count($route) > 0) {
            if (self::is_controller(join("/", $route))) {
    ...
    {
        if (class_exists(Inflections::slashcamelize(str_replace("-", "", $test), true) . "Controller",
            self::$autoload)) {
            return true;
        }
        $path = "";
    ...
    protected function detect_maintenance()
    {
        ...
        $redirect = false;
        //maintenance is setup
        if ($maintenance['ip'] && $maintenance['redirect']) {
            $redirect = true;
            //if an exclusion ip is set the check the remote address - reset the flag
            if (is_array($maintenance['ip'])) {
                foreach ($maintenance['ip'] as $ip) {
                    if (preg_match("/" . preg_quote($ip) . "/i", $_SERVER['REMOTE_ADDR'])) {
                        $redirect = false;
                    }
                }
            } elseif (preg_match("/" . preg_quote($maintenance['ip']) . "/i", $_SERVER['REMOTE_ADDR'])) {
                $redirect = false;
            }
        }
        if ($redirect) {
