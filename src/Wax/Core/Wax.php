...
class Wax {
  ...
  public static function register_file($class, $file) {
    self::$registered_classes[$class] = $file;
  }
  ...
  static public function include_from_registry($class){
    if(self::$registered_classes[$class] && !self::$loaded_classes[$class]){
      ...
      if(!self::$registered_classes[$class]) Wax::load("form");
      if(!self::$registered_classes[$class]) Wax::load("form",APP_DIR);
  ...
  public function pre_initialise() {
    if(defined("WAX_PROFILE")) {
		  self::load("utilities/Profiler");
      Profiler::profile();
		}    
		Event::run("wax.pre_init");
    self::constants();
