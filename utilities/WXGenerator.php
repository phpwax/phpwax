...
class WXGenerator {
  ...
  public function new_test($args=array()) {
		$class = "Test".Inflections::camelize($args[0], true);
  ...
  public function new_model($args=array()) {
		$class = Inflections::camelize($args[0], true);
    ...
    $res = $this->write_to_file(APP_DIR."model/".$class.".php");
		...
		$this->final_output="";
		$this->new_migration("create_".Inflections::underscore($class), Inflections::underscore($class) );
  }
  ...
  public function new_controller($args=array()) {
		$path = explode("/", $args[0]);
    $class = Inflections::camelize(implode("_", $path), true)."Controller";
    if(count($path)> 1) {
     	$path = $path[0]."/";
  ...
  public function new_email($args=array()) {
    ...
    $res = $this->write_to_file(APP_DIR."model/".$class.".php");
		...
		$this->make_view(Inflections::underscore($class));
  }
  ...
  public function new_migration($name, $table=null) {
    ...
    $class = Inflections::camelize($name, true);
    ...
    $file = str_pad($version, 3, "0", STR_PAD_LEFT)."_".Inflections::underscore($class);
