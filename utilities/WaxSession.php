...
class WaxSession {
    ...
    function __construct($data = array()){
        ...
        if(($this->id = $_COOKIE[$this->name]) || ($this->id = $_REQUEST[$this->name])){
            ...
            if(!static::$data[$this->name] && (is_readable($storage = $this->file_storage())) && ($stats = stat($storage))){
                if(time() < $stats[9]) {
                    static::$data[$this->name] = unserialize(file_get_contents($this->file_storage()));
                }
                else {
            ...
            $this->id = $this->safe_encrypt($_SERVER['REMOTE_ADDR'].microtime().mt_rand());
        }

        //add folder to collection of folders to be garbage collected
        static::$garbage_collection_folders[] = $this->file_storage_dir();

        //set cookie on render to propagate session
        $session = $this;
        WaxEvent::add(
            'wax.response.execute', function() use ($session) {
            $response = WaxEvent::data();
            $expires = $session->lifetime ? (time() + $session->lifetime) : false;
            $response->set_cookie($session->name, $session->id, $expires);
        });

        $session_save = $this;
        WaxEvent::add(
            'wax.response.execute', function () use ($session_save){
    ...
    public function save_session(){
        ...
        if(!is_dir($this->file_storage_dir())){
            if(!mkdir($this->file_storage_dir(), 0750, true)) throw new WaxException(
                'Session not writable - ' . $this->file_storage_dir());
        }
    ...
    public function get($key)
    {
        ...
        if (isset(static::$data[$this->name][$key])) {
...
if($argv){
    ...
    foreach($argv as $dir){
        ...
        foreach(glob("$dir/*") as $file){
            if($file === "$dir/garbage.collect.lock") {
                continue;
            }
