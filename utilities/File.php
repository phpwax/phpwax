...
class File {
  ...
  static $resize_library = "gd"; // Optionally set to 'gd'
	...
	/**
	  ...
	  * @return bool
	  */
	static function resize_image($source, $destination, $width, $overwrite=false, $force_width=false, $force_img_magick=false) {
		if(!self::is_image($source)) return false;
	  if(!$force_img_magick && self::$resize_library == "gd" && function_exists("imagecreatefromjpeg")) return self::gd_resize_image($source, $destination, $width, $overwrite=false, $force_width=false);
