...
class File
{
    ...
    public static $compression_quality = '85';
    public static $resize_library = 'gd'; // Optionally set to 'gd'
     ...
     * @return bool|void
     */
    public static function smart_resize_image($source, $destination, $width, $height, $mode = 'crop')
    {
        ...
        if ((!$width && !$height) || !function_exists('imagecopyresampled')) {
            return false;
        }

        if (!$height || !$width) {
            $mode = 'nocrop';
        } //force nocrop when specifying only 1 dimension
        ...
        if ($mode == "small") {
            if ($r_h < $r_w) {
                $width = $r_h * $source_width;
            } //ignore target width and use the aspect ratio to work it out
            else {
                $height = $r_w * $source_height;
            } //ignore target height and use the aspect ratio to work it out
    ...
    {
        if (!self::is_image($source)) {
            return false;
        }
        if (self::$resize_library == "gd" && function_exists("imagecreatefromjpeg")) {
            return self::gd_resize_image($source, $destination, $width, $overwrite = false, $force_width = false);
        }
        ...
        if ($ratio == 1) {
            ...
            $command = "mogrify ".escapeshellarg(
                    $source
                )." -limit area 30 -render -flatten -coalesce -colorspace RGB -resize {$width}x{$height} -quality ".self::$compression_quality;
        } else {
            $command = "convert ".escapeshellarg(
                    $source
                )." -limit area 30 -coalesce -thumbnail {$width}x{$height} -density 72x72 -quality ".self::$compression_quality."  $destination";
        }
        system($command);
    ...
    static private function output_image_gd($image_type, $dst, $destination)
    {
        switch ($image_type) {
            case "gif":
            case 1:
                $src = imagegif($dst, $destination);
                break;
            case "jpg":
            case "jpeg":
            case 2:
                $src = imagejpeg($dst, $destination, self::$compression_quality);
                break;
            case "png":
            case 3:
                $src = imagepng($dst, $destination);
                break;
        }
    ...
    static function rotate_image($source, $destination, $angle)
    {
        ...
        if (self::$resize_library == "gd" && function_exists("imagerotate")) {
            return self::gd_rotate_image($source, $destination, $angle);
        }
    ...
    static function resize_image_extra(
        $source,
        $destination,
        $percent = false,
        $x = false,
    ...
    static function crop_image($source, $destination, $x, $y, $width, $height)
    {
        ...
        if (self::$resize_library == "gd" && function_exists("imagecopyresampled")) {
            return self::gd_crop_image($source, $destination, $x, $y, $width, $height);
        }
        system("cp $source $destination");
    ...
    static function gd_crop_image($source, $destination, $x, $y, $width, $height)
    {
        ...
        imagesavealpha($dst, true);
        imagefill($dst, 0, 0, imagecolorallocatealpha($dst, 255, 255, 255, 127));
        if (!imagecopyresampled($dst, $src, 0, 0, $x, $y, $width, $height, $width, $height)) {
            return false;
        }
    ...
    static function recursively_delete($item)
    {
        ...
        foreach ($iter as $file) {
            if ($iter->isDir() && is_readable($file) && !$iter->isDot()) {
                rmdir($file);
            } elseif ($iter->isFile() && is_readable($file)) {
                unlink($file);
            }
        }
        if (is_dir($item) && is_readable($item) && substr($item, -1) != ".") {
            rmdir($item);
        }
        ...
        foreach ($dir as $file) {
            if (!strstr($dir->getPath()."/".$file, "/.")) {
                if (self::is_image($file)) {
                    $imagearray[] = ["filename" => $dir->getFilename(), "path" => base64_encode($file)];
                }
            }
        }
    ...
    static function display_asset($path, $mime = null)
    {
        if (!is_readable($path)) {
            return false;
        }
    ...
    public static function detect_mime($file)
    {
        if ($res = self::mime_map($file)) {
            $type = $res;
    ...
    static function recursive_directory_copy($source, $destination, $verbose = true)
    {
        ...
        foreach (File::scandir($source, ".htaccess") as $file) {
            if (is_file($source.DIRECTORY_SEPARATOR.$file)) {
                ...
                if ($verbose) {
                    echo "..created ".$destination.DIRECTORY_SEPARATOR.$file."\n";
                }
            } else {
                File::recursive_directory_copy(
    ...
    public static function scandir($directory, $include_pattern = false)
    {
        $list = [];
        foreach (scandir($directory) as $item) {
            if (preg_match("/".$include_pattern."/", $item)) {
                $list[] = $item;
