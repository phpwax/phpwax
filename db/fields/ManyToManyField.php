...
class ManyToManyField extends WaxModelField
{
    ...
    public function setup_join_model()
    {
        ...
        $this->join_model = $join->filter([$this->join_field($this->model) => $this->model->primval]);
        if ($this->join_order) {
            $this->join_model->order($this->join_order);
        }
    }

    /**
     * take the model and create a string version of the field to use in the join
     * @param WaxModel $model
     * @return string
     */
     ...
     * @return void|WaxRecordset
     * @throws WXException
     */
    public function set($value)
    {
        if (!$this->model->primval) {
            throw new WXException("ManyToMany set before Model Save",
                "Cannot set " . get_class($this->model) . "->" . $this->field . " before saving " . get_class($this->model));
        ...
        if ($value instanceof WaxModel) {
            if (!($ret = $this->join_model->filter([$this->join_field($value) => $value->primval])->first())) {

                $new = [
                    $this->join_field($value) => $value->primval,
                    $this->join_field($this->model) => $this->model->primval,
                ];
                foreach (array_diff_key($this->join_model->columns, [
                    $this->join_model->primary_key => false,
                    $this->join_model->left_field => false,
                    $this->join_model->right_field => false,
                ]) as $col => $col_options) {
                    $new[$col] = $value->$col;
                }
    ...
    public function unlink($model = false)
    {
        ...
        } //if we don't know what to unlink from we can't unlink anything, just do nothing and return
        WaxModel::unset_cache(get_class($this->model), $this->field);
        if (!$model) {
            $model = $this->get();
        } //if nothing gets passed in to unlink then unlink everything
        ...
        if ($model instanceof WaxRecordset) {
            ...
            if (count($filter)) {
                $this->join_model->filter($links->table . "_" . $links->primary_key, $filter)->delete();
            }
        } else {
    ...
    public function get($filters = false)
    {
        ...
        if (!$this->model->primval) {
            return new WaxModelAssociation($this->model, new $this->target_model, [], $this->field);
        }
        if ($this->eager_loading) {
            return $this->eager_load($target);
        }
    ...
    protected function eager_load($target)
    {
        ...
        $this->join_model->select_columns = [$target->table . ".*"];
        foreach (array_diff_key($this->join_model->columns, [
            $this->join_model->primary_key => false,
            $this->join_model->left_field => false,
            $this->join_model->right_field => false,
    ...
    protected function lazy_load($target_model)
    {
        ...
        $ids = [];
        if ($cache = WaxModel::get_cache(get_class($this->model), $this->field,
            $this->model->primval . ":" . md5(serialize($target_model->filters)), false)) {
        ...
        WaxModel::set_cache(get_class($this->model), $this->field,
            $this->model->primval . ":" . md5(serialize($target_model->filters)), $ids);
        ...
        if (is_array($params)) {
            foreach ($params as $column => $value) {
                if (!$this->join_model->columns[$column]) {
                    return $this->__call("filter", [$params, $value, $operator]);
                }
            }
    ...
    public function __call($method, $args)
    {
        ...
        if ($this->eager_loading) {
            ...
            foreach ((array)$assoc->rowset as $row) {
                $filter_args[$key][] = $row[$key];
            }
        } else {
            $filter_args = [$model->primary_key => $assoc->rowset];
        }
        if ($assoc->rowset) {
            $model->filter($filter_args);
        } else {
            $model->filter("1 = 2");
        }
