...
class DateTimeField extends WaxModelField
{
    ...
    public function setup()
    {
        if (!$this->input_format) {
            $this->input_format = $this->output_format;
        }
        if (isset($this->model->row[$this->field]) && $this->model->row[$this->field] == 0 && is_string($this->default)) {
            $this->model->row[$this->field] = date($this->save_format, strtotime($this->default));
        }
        if ($this->required) {
            $this->validations["datetime"];
        }
    }
    ...
    {
        if (($ret = strtotime(str_replace("/", "-", $val))) !== false) {
