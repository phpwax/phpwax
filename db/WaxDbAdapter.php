...
abstract class WaxDbAdapter
{
    ...
    public function __construct($db_settings = array())
    {
        ...
        if (!$db_settings['dbtype']) {
            $db_settings['dbtype'] = "mysql";
        }
        if (!$db_settings['host']) {
            ...
            switch ($e->getCode()) {
                ...
                case "42S22":
                    ...
                    try {
                        ...
                        throw new WaxDBStructureException("{$err[2]}", "Database Structure Error",
                            $pdo_statement->queryString . "\n" . print_r($bindings, 1));
                    }
                    break;
                default:
                    WaxLog::log("error",
                        "[DB] $err[2] , SQL: " . $pdo_statement->queryString . ($bindings ? "\n" . print_r($bindings,
                                1) : ""));
                    if (!$swallow_errors) {
                        throw new WaxSqlException("{$err[2]}", "Error Preparing Database Query",
                            $pdo_statement->queryString . "\n" . print_r($bindings, 1));
                    }
            }
        }
    ...
    public function update_sql($model)
    {
        ...
        return "UPDATE `{$model->table}` SET " . $this->update_values(array_intersect_key($model->row,
            $model->_col_names)) .
    ...
    public function delete_sql($model)
    {
        ...
        if ($model->primval()) {
            $sql .= " WHERE {$model->primary_key}={$model->primval()}";
        }
    ...
    public function left_join($model)
    {
        if ($model->is_left_joined && count($model->join_conditions)) {
            ...
            if ($model->left_join_target instanceof WaxModel) {
                $join_table = $model->left_join_target->table;
            } else {
                $join_table = $model->left_join_target;
            }
    ...
    {
        if ($model->group_by) {
            return array("sql" => " GROUP BY {$model->group_by}", "params" => $model->_group_by_params);
        }
    }
    ...
    {
        if ($model->_order) {
            return array("sql" => " ORDER BY " . $model->_order, "params" => $model->_order_params);
        }
    }
    ...
    {
        if ($model->_limit) {
            return " LIMIT " . (int)$model->_offset . ", " . (int)$model->_limit;
        }
    }
        ...
        if (count($model->$filter_name)) {
            foreach ($model->$filter_name as $filter) {
                if (is_array($filter)) {
                    ...
                    if (in_array($filter["name"], array_keys($model->columns))) {
                        $sql .= "`$model->table`.";
                    }
                    $sql .= $filter["name"] . $this->operators[$filter["operator"]] . $this->map_operator_value($filter["operator"],
                            $filter["value"]);
    ...
    public function syncdb(WaxModel $model)
    {
        ...
        foreach ($tables as $table) {
            if ($table[0] == $model->table) {
                $exists = true;
            }
        }
        if (!$exists && $info = $this->create_table($model)) {
            $output .= $info . "\n";
        }
        ...
        foreach ($model->columns as $model_col => $model_col_setup) {
            ...
            if ($info = $model_field->before_sync()) {
                $output .= $info;
            }
            $col_exists = false;
            $col_changed = false;
            foreach ($db_cols as $key => $col) {
                if ($col["COLUMN_NAME"] == $model_field->col_name) {
                    $col_exists = true;
                    if ($col["COLUMN_DEFAULT"] != $model_field->default) {
                        $col_changed = "default";
                    }
                    if ($col["IS_NULLABLE"] == "NO" && $model_field->null) {
                        $col_changed = "now null";
                    }
                    if ($col["IS_NULLABLE"] == "YES" && !$model_field->null) {
                        $col_changed = "now not null";
                    }
                }
            }
            if ($col_exists == false && ($info = $this->add_column($model_field, $model, true))) {
                $output .= $info;
            }
            if ($col_changed && ($info = $this->alter_column($model_field, $model, true))) {
                $output .= $info . " " . $col_changed;
            }
        }
    ...
    public function column_sql(WaxModelField $field, WaxModel $model)
    {
        ...
        if ($type == "string" && !$field->maxlength) {
            $sql .= "(255) ";
        ...
        if ($field->default || $field->database_default) {
    ...
    protected function map_operator_value($operator, $value)
    {
        switch ($operator) {
            ...
            case "LIKE":
                return " LIKE ?";
            case "in":
                return "(" . rtrim(str_repeat("?,", count($value)), ",") . ")";
            case "raw":
                return "";
            default:
                return "?";
